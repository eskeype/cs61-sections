...
void fast_matrix_multiply(double* c, size_t sz, double* a, double* b) {
    ...
    for (size_t i = 0; i < sz; ++i)
        for (size_t j = 0; j < i; ++j) {
            double tmp = *me(b, sz, i, j);
            *me(b, sz, i, j) = *me(b, sz, j, i);
            *me(b, sz, j, i) = tmp;
        }
    ...
    for (size_t i = 0; i < sz; ++i)
        for (size_t j = 0; j < sz; ++j)
            for (size_t k = 0; k < sz; ++k)
                *me(c, sz, i, j) += *me(a, sz, i, k) * *me(b, sz, j, k);
...
int main(int argc, char* argv[]) {
    ...
    // read options
    int opt;
    while ((opt = getopt(argc, argv, "n:d:t:")) != -1)
        switch (opt) {
        case 'n':
        ...
        case 'd':
            ...
            break;
        case 't':
            basetime = strtod(optarg, NULL);
            break;
        default:
            fprintf(stderr, "Usage: ./matrixmultiply [-n SIZE] [-d SEED] [-t BASETIME]\n");
            exit(EXIT_FAILURE);
        }
    ...
    // if we have the base time, exit early
    if (basetime) {
        printf("assuming base took %ld.%06lds\n", (long) basetime, (long) (1000000 * (basetime - (long) basetime)));
